...
class _TableCalendarState extends State<TableCalendar> with SingleTickerProviderStateMixin {
  ...
  void _onDayLongPressed(DateTime day) {
    if (widget.onDayLongPressed != null) {
      widget.onDayLongPressed(day, widget.calendarController.visibleEvents[_getEventKey(day)] ?? [],  widget.calendarController.visibleMarks[_getMarkKey(day)] ?? []);
    }
  }
  ...
  Widget _buildCell(DateTime date) {
    ...
    final key = eventKey ?? markKey ?? holidayKey;

    if (key != null) {
      final children = <Widget>[content];
      final events = eventKey != null ? widget.calendarController.visibleEvents[eventKey] : [];
      final marks = markKey != null ? widget.calendarController.visibleMarks[markKey] : [];
  ...
  Widget _buildCellContent(DateTime date) {
    ...
    if (isUnavailable) {
      return widget.builders.unavailableDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isSelected && widget.calendarStyle.renderSelectedFirst) {
      return widget.builders.selectedDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isToday) {
      return widget.builders.todayDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isSelected) {
      return widget.builders.selectedDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isOutsideHoliday) {
      return widget.builders.outsideHolidayDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isHoliday) {
      return widget.builders.holidayDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isOutsideWeekend) {
      return widget.builders.outsideWeekendDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isOutside) {
      return widget.builders.outsideDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (isWeekend) {
      return widget.builders.weekendDayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else if (widget.builders.dayBuilder != null) {
      return widget.builders.dayBuilder(context, date, widget.calendarController.visibleEvents[eventKey], widget.calendarController.visibleMarks[markKey]);
    } else {
      return _CellWidget(
        text: '${date.day}',
  ...
  Widget _buildMarker2(DateTime date, dynamic mark) {
    if (widget.builders.singleMarkerBuilder != null) {
      return widget.builders.singleMarkerBuilder(context, date, mark);
    } else {
      return Container(
        width: 8.0,
        height: 8.0,
